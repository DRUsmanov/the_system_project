...
std::optional<domain::User> UserRepository::LoginUser(const domain::UserLoginData& login_data) const {
    pqxx::work work(uow_->GetConnection());
    auto result = work.exec_params(query::login, login_data.login, login_data.password);
