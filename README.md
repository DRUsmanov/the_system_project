...
+ Удаление работника
+ Изменение работника
+ Получение табеля рабочего времени
...
admin_categorys ||--o| users : "один пользователя -> ноль или одна категория администраторов"
```
...
LoginRequestHandler ->> TokenManager: payload

activate TokenManager
TokenManager -->> LoginRequestHandler: token
...
box rgb(138, 136, 252) Infrastructure
participant RequestHandler
participant ShopRequestHandler
participant EmployeeRequestHandler
participant TokenManager
end
...
activate RequestHandler
RequestHandler ->> TokenManager: request

activate TokenManager

alt Токен валиден
TokenManager -->> RequestHandler: payload

RequestHandler ->> ShopRequestHandler: add_employee_request + payload

activate ShopRequestHandler
ShopRequestHandler ->> EmployeeRequestHandler: add_employee_request + payload
deactivate ShopRequestHandler

activate EmployeeRequestHandler
EmployeeRequestHandler ->> ApplicationManagerInterface: EmployeeInputDto + UserIdInputDto
...
ApplicationManagerInterface -->> EmployeeRequestHandler: success

EmployeeRequestHandler -->> user: 201 + message("Работник успешно добавлен")
...
ApplicationManagerInterface -->> EmployeeRequestHandler: unsuccess
deactivate ApplicationManagerInterface

EmployeeRequestHandler -->> user: 500 + message(причина)
deactivate EmployeeRequestHandler

end

else Токен невалиден

TokenManager -->> RequestHandler: invalid_token
deactivate TokenManager

RequestHandler -->> user: 401 + login.html
deactivate RequestHandler
...
RequestHandler ->> ShopRequestHandler: delete_employee_request + payload

activate ShopRequestHandler
ShopRequestHandler ->> EmployeeRequestHandler: delete_employee_request + payload

activate EmployeeRequestHandler
EmployeeRequestHandler ->> ApplicationManagerInterface: EmployeeIdInputDto + UserIdInputDto
...
EmployeeRequestHandler -->> user: 201 + message("Работник успешно удален")
...
RequestHandler ->> ShopRequestHandler: change_employee_reques + payload

activate ShopRequestHandler
ShopRequestHandler ->> EmployeeRequestHandler: change_employee_reques + payload

activate EmployeeRequestHandler
EmployeeRequestHandler ->> ApplicationManagerInterface: EmployeeIdInputDto + EmployeeInputDto + UserIdInputDto
...
EmployeeRequestHandler -->> user: 201 + message("Работник успешно изменен")
